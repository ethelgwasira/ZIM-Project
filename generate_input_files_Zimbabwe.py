...
def generate_demographics(demo_df, demo_fname):
    ...
    br_concern = WorldBankBirthRateConcern(country="Mozambique", birthrate_year=2016)
...
if __name__ == '__main__':
    inputs_path = os.path.join('./', 'input/Mozambique')
    if not os.path.exists(inputs_path):
        os.mkdir(inputs_path)

    df = pd.DataFrame(data={'nodeid': [1], 'population': [1400], 'Village': ['Mudzi'],
