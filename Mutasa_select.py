...
def plot_output(sim_df, data_df, score_df, variable):
    ...
    score_df1 = score_df[score_df.ll == score_df.ll.max()].reset_index()
    candidate_val = score_df[variable]

    for cand in candidate_val:
        plot_df = sim_df[sim_df[variable] == cand]
        a = 1 if cand == score_df1[variable][0] else 0.08
        axes[0].plot(plot_df['date'], plot_df['PfPR'], color='#FF0000', alpha=a)
        # axes[0].plot(plot_df['year'], plot_df['PfPR'], color='#FF0000', alpha=a)
    ...
    axes[1].scatter(score_df1[variable], score_df1.ll, s=90, color='blue')
